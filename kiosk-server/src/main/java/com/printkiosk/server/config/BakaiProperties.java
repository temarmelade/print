...
public class BakaiProperties {
    ...
    /**
     * Готовый токен, полученный вручную (например, через Swagger).
     *
     * <p>Нужен, если учётные данные из PDF одноразовые: тогда повторный
     * вызов /Auth/Login возвращает 400, и логиниться из кода нельзя в
     * принципе. Заполнен — вход не выполняется вообще, используется этот
     * токен. Пусто — обычная авторизация по логину и паролю.
     */
    private String token;

    /**
     * Метод генерации QR. Токен выдаётся под конкретную услугу, поэтому
     * значение должно совпадать с тем, под что создан внешний сервис в
     * Bakai Business. Обращение к другому методу вернёт 401.
     */
    private String generateQrPath = "/api/Qr/GenerateQR";
